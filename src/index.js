...
if (data.length === 0) {
  ...
  if (lawn.length === 0) {
    ...
    while (currentRowInDataFile < data.length) {
      ...
      const isValidPositionArray = initialMowerPositionAsIntArray.every(element => {
        return typeof element === 'number';
      });

      if (!isValidPositionArray) {
        console.error(`Invalid initial position array provided. ${initialMowerPositionAsIntArray} should be array of two integers`)
      }
